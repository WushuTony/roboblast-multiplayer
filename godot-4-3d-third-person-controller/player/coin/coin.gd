...
@onready var _coin_visuals: Node3D = $Coin
...
func spawn(coin_delay: float = 0.5) -> void:
	...
	_apply_spawn_impulse.rpc(rand_pos)
...
func _apply_spawn_impulse(impulse: Vector3):
...
func _on_body_entered(body: PhysicsBody3D) -> void:
	if _target == null and body is Player:
		_set_target.rpc(body.get_path())


@rpc("authority", "call_local", "reliable")
func _set_target(path: NodePath):
	var new_target: Player = get_node(path)
	if new_target != null:
		_on_target_player(new_target)


func _on_target_player(new_target: Player) -> void:
...
func _collect() -> void:
	...
	Level.play_sound(_collect_audio, global_position, randfn(1.0, 0.1))

	if is_instance_valid(_target) and _target.is_multiplayer_authority():
		_target.collect_coins()
